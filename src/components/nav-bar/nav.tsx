...
const Nav = (props: Props) => {
  ...
  return (
    <div
      className=
        "bg-[var(--color-true-white)] z-100 w-full pt-2 px-6 pb-2  flex flex-row items-center justify-between sticky max-w-[1440px] mx-auto"
        // className
    ...
    >
      ...
      {isOpen && (
        <div className="absolute top-12 left-0 w-full bg-white shadow-lg rounded-md flex flex-col gap-2 text-[var(--color-grey-400)] font-[var(--font-sans)] text-base p-4 z-100">
          <Link href="/" className={`p-2.5 hover:bg-[var(--color-purple-blue)] hover:text-white ${pathname === "/" ? "text-[var(--color-purple-blue)]" : ""}`} onClick={() => setIsOpen(false)}>Flights</Link>
          <Link href="/hotels" className={`p-2.5 hover:bg-[var(--color-purple-blue)] hover:text-white ${pathname === "/hotels" ? "text-[var(--color-purple-blue)]" : ""}`} onClick={() => setIsOpen(false)}>Hotels</Link>
          <Link href="/packages" className={`p-2.5 hover:bg-[var(--color-purple-blue)] hover:text-white ${pathname === "/packages" ? "text-[var(--color-purple-blue)]" : ""}`} onClick={() => setIsOpen(false)}>Packages</Link>
          <button onClick={() => {
            setIsModalOpen(true);
            setIsOpen(false);
          }}  className={`p-2.5 hover:bg-[var(--color-purple-blue)] hover:text-white ${pathname === "/signIn" ? "text-[var(--color-purple-blue)]" : ""}`}>Sign in</button>
          <button onClick={() => {
            setIsModalOpen(true);
            setIsOpen(false);
          }}  className="bg-[var(--color-purple-blue)] text-white rounded p-3 text-center">Sign up</button>
        </div>
      )}
      ...
      <div className="p-4 hidden lg:flex flex-row gap-4 text-[var(--color-grey-400)] font-[var(--font-sans)] text-base items-center justify-start shrink-0 relative">
        ...
        <Link href='/hotels' className="hover:scale-105 p-2.5 flex flex-row gap-2.5 items-center justify-start shrink-0 relative overflow-hidden">
        <div className={`${ pathname === '/hotels' ? 'text-[var(--color-purple-blue)]' : ''}
           text-center relative flex items-center justify-center`}>
            Hotels{" "}
          </div>
        </Link>
        <Link href='/packages' className="hover:scale-105 p-2.5 flex flex-row gap-2.5 items-center justify-start shrink-0 relative overflow-hidden">
        <div className={`${ pathname === '/packages' ? 'text-[var(--color-purple-blue)]' : ''}
           text-center relative flex items-center justify-center`}>
            Packages{" "}
          </div>
        </Link>
